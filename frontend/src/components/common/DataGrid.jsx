...
export const DataGrid = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm flex flex-col transition-colors w-full">
      {/* Header toolbar */}
      {(title || actionButton || subtitle) && (
        <div className="px-3 sm:px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 bg-slate-50 dark:bg-slate-900/50">
          <div>
            {title && <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">{title}</h4>}
            {subtitle && <p className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{subtitle}</p>}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative flex-1 sm:flex-initial">
              ...
              <input
                ...
                className="pl-7 pr-2.5 py-1 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-lg text-xs text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-slate-900 dark:focus:border-sky-500 w-full sm:w-44"
              />
            </div>
            {actionButton}
          </div>
      ...
      {/* Pagination Toolbar */}
      <div className="px-3 sm:px-4 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-600 dark:text-slate-400">
